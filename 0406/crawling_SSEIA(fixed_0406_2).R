...
for(i in 1:length(sVal)){
  ...
  for(pn in 1:pn_total){
    ...
    for(col in 1:col_total){
      ...
      # 4) 사업 구분
      # CSS 추출
      #sub_con > div.conbody > ul.tab_cont > li.on > table > tbody > tr:nth-child(1) > td:nth-child(2)
      prop <- remDr$findElements(using ="css selector","li.on > table > tbody > tr:nth-child(1) > td:nth-child(2)")
      ...
      # 5) 사업 위치
      # CSS 추출
      loc <- remDr$findElements(using = "css selector","li.on > table > tbody > tr:nth-child(2) > td:nth-child(2) > table > tbody > tr > td:nth-child(1)")
      ...
      # 6) 사업규모
      # CSS 추출
      area <- remDr$findElements(using ="css selector","li.on > table > tbody > tr:nth-child(6) > td:nth-child(2)")
  ...
  fileName <- paste0("소규모환경영향평가_",sVector[sIndex],".csv")
