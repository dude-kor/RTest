...
eia<-remDr$findElements(using ="css selector","#visual_wrap > div > ul > li:nth-child(3) > a")

# 버튼 클릭
sapply(eia,function(x){x$clickElement()})
...
for(sIndex in 1:length(sVector)){
  ...
  # 검색 결과 수 추출
  colNum <- remDr$findElements(using ="css selector","#sub_con > div.conbody > div > span")
  ...
  for(page_tenth in 1:page_ten){
    ...
    for(page in 1:page_index){
      ...
      for(i in 1:index_ten){
        print(index)
        # 사업명
        cssSelector <- paste0("#sub_con > div.conbody > div > table > tbody > tr:nth-child(",i,") > td.title")
        ...
        # 1) 사업코드 
        # CSS 추출
        code <- remDr$findElements(using ="css selector","#sub_con > div.conbody > div.sub_view_wrap > table > tbody > tr:nth-child(1) > td:nth-child(2)")
        ...
        # 2) 사업명 
        # CSS 추출
        name <- remDr$findElements(using ="css selector","#sub_con > div.conbody > div.sub_view_wrap > table > tbody > tr:nth-child(2) > td:nth-child(2)")
        ...
        # 3) 사업등록 유형 
        # CSS 추출
        type <- remDr$findElements(using ="css selector","#sub_con > div.conbody > div.sub_view_wrap > table > tbody > tr:nth-child(3) > td:nth-child(2)")
        ...
        # 4) 사업 구분
        # CSS 추출
        prop <- remDr$findElements(using ="css selector","#infos_eiass > ul.tab_cont > li.on > table > tbody > tr:nth-child(2) > td:nth-child(2)")
        ...
        # 5) 사업 위치
        # CSS 추출
        loc <- remDr$findElements(using = "css selector","#infos_eiass > ul.tab_cont > li.on > table > tbody > tr:nth-child(1) > td:nth-child(2) > table > tbody > tr > td")
        ...
        # 6) 사업규모
        # CSS 추출
        area <- remDr$findElements(using ="css selector","#infos_eiass > ul.tab_cont > li.on > table > tbody > tr:nth-child(5) > td:nth-child(2) > table > tbody > tr > td")
